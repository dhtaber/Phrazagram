# word_grid_solver_general_V6.py
...
# -----------------------------
DEFAULT_DICT_PATH = r"C:\Alloquest_Photos\Word_Game\my-word-game\tools\latest_3.txt"
OUTPUT_DIR = "./solver_outputs"
DEFAULT_MAX_GRIDS = 25
DEFAULT_SOLUTIONS_PER_GRID = 5
PER_GRID_TIME_LIMIT = 15          # seconds per grid (soft cap for search)
...
def clean_phrase(p: str) -> str:
    return "".join(ch for ch in p.upper() if "A" <= ch <= "Z")
...
def main():
    ...
    L = len(letters)
    words_by_len = load_dictionary(args.dict)
