# word_grid_solver_general_V11.py  (revised)
...
DEFAULT_SOLUTIONS_PER_GRID = 25
PER_GRID_TIME_LIMIT = 20         # seconds per grid (secondary/deepen phase cap)
DEFAULT_MIN_INTERSECTIONS = 0   # total crossing cells threshold; 0 = no extra filter
HEARTBEAT_SEC = 1.0

# NEW: fast probe phase (Phase A)
PROBE_TIME_SEC = 2
PROBE_MAX_SOLUTIONS = 1

# NEW: global solutions cap via constant (0 = off). CLI --total_solutions_cap overrides if set.
GLOBAL_SOLUTIONS_CAP = 0

# NEW: suppress solver heartbeats by default
...
# NEW: build the set of banned consecutive substrings of the phrase (lengths 4..7).
def build_banned_substrings(letters: str, kmin: int = 4, kmax: int = 7) -> set[str]:
    """
    From a normalized letters-only phrase (A..Z), build a set of all
    consecutive substrings of length kmin..kmax.
    """
    banned = set()
    n = len(letters)
...
def load_dictionary(path: str, min_rating: int):
    ...
    best_by_cleaned = {}  # cleaned_word -> highest_rating_or_None
...
# Helper to dedupe solution lists by class signature (outside the solver)
def solution_signature(sol, sid_to_class):
    buckets = defaultdict(list)
    for sid, word in sol.items():
...
def dedupe_solution_list(solutions, sid_to_class):
    seen = set()
    out = []
    for sol in solutions:
        sig = solution_signature(sol, sid_to_class)
        if sig not in seen:
            seen.add(sig)
            out.append(sol)
    return out, seen
...
def main():
    ...
    ap.add_argument("--per_grid_time", type=int, default=PER_GRID_TIME_LIMIT, help="Seconds per grid (soft) for DEEPEN phase.")
    ...
    ap.add_argument("--total_solutions_cap", type=int, default=0,
                    help="Stop after this many total solutions across all grids (0 = no cap). Overrides GLOBAL_SOLUTIONS_CAP if >0.")
    args = ap.parse_args()
    ...
    # REMOVED: interactive global-cap prompt. Use CLI flag or constant.
    # Effective global cap is the minimum of (CLI cap if >0) and (GLOBAL_SOLUTIONS_CAP if >0), else unlimited.
    cli_cap = args.total_solutions_cap if args.total_solutions_cap and args.total_solutions_cap > 0 else None
    const_cap = GLOBAL_SOLUTIONS_CAP if GLOBAL_SOLUTIONS_CAP and GLOBAL_SOLUTIONS_CAP > 0 else None

    def remaining_global_cap(total_so_far: int) -> int | None:
        caps = []
        if cli_cap is not None: caps.append(cli_cap)
        if const_cap is not None: caps.append(const_cap)
        if not caps:
            return None  # unlimited
        eff_cap = min(caps)
        rem = eff_cap - total_so_far
        return max(0, rem)
    ...
    # Load dictionary (keeps only 4..7 letter cleaned entries)
    words_by_len = load_dictionary(args.dict, min_rating)

    # === NEW: build banned 4..7 letter consecutive substrings from the phrase, and filter dictionary ===
    banned_substrings = build_banned_substrings(letters, 4, 7)
    removed_total = 0
    for _L in range(4, 8):
        if _L in words_by_len:
            before = len(words_by_len[_L])
            # If dictionary words are stored raw, ensure we compare normalized forms.
            words_by_len[_L] = [w for w in words_by_len[_L] if w not in banned_substrings]
            removed_total += (before - len(words_by_len[_L]))
    if removed_total > 0:
        print(f"[filter] Removed {removed_total} word(s) that appear as exact 4–7 letter consecutive substrings of the phrase.")
    ...
    modes_str = "5&6" if args.words_mode == "both" else args.words_mode
    print(f"Prepared {len(selected)} of {total_candidates} possible {L}-letter grids "
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        for idx, layout in enumerate(selected, 1):
            ...
            # Prefilter candidates; skip grids with empty slots immediately.
            budget = Counter(letters)
            cand = prefilter_candidates(words_by_len, slots, budget)
            if any(len(cand[s['id']]) == 0 for s in slots):
                continue

            # Determine remaining global capacity (if any)
            rem_global = remaining_global_cap(total_solutions)
            if rem_global is not None and rem_global <= 0:
                # Already at cap; stop early
                if cli_cap is not None and total_solutions >= cli_cap:
                    print(f"Reached CLI global cap of {cli_cap} solutions. Stopping early.")
                elif const_cap is not None and total_solutions >= const_cap:
                    print(f"Reached global cap of {const_cap} solutions. Stopping early.")
                break

            # ---------------- Phase A: PROBE ----------------
            quiet = (args.quiet or SUPPRESS_HEARTBEAT_LOGS)
            grid_remaining_cap = args.solutions_per_grid
            if rem_global is not None:
                grid_remaining_cap = min(grid_remaining_cap, rem_global)
            probe_cap = min(PROBE_MAX_SOLUTIONS, grid_remaining_cap)
            if probe_cap <= 0:
                continue

            probe_solutions = find_solutions_for_grid(
                slots, intersections, letters, cand,
                per_grid_time=PROBE_TIME_SEC,
                max_solutions=probe_cap,
                quiet=quiet
            )

            if not probe_solutions:
                # No print, no file output for unsolved grid
                continue

            # ---------------- Phase B: DEEPEN ----------------
            # Compute remaining caps after probe
            rem_global = remaining_global_cap(total_solutions + len(probe_solutions))
            grid_remaining_cap = args.solutions_per_grid - len(probe_solutions)
            if rem_global is not None:
                grid_remaining_cap = min(grid_remaining_cap, rem_global)
            deepen_cap = max(0, grid_remaining_cap)

            deepen_solutions = []
            if deepen_cap > 0 and args.per_grid_time and args.per_grid_time > 0:
                deepen_solutions = find_solutions_for_grid(
                    slots, intersections, letters, cand,
                    per_grid_time=args.per_grid_time,
                    max_solutions=deepen_cap,
                    quiet=quiet
                )

            # Combine & dedupe (by equivalence class signature)
            _, sid_to_class = slot_equivalence_classes(slots, intersections)
            combined = probe_solutions + deepen_solutions
            combined_unique, _seen = dedupe_solution_list(combined, sid_to_class)

            if not combined_unique:
                continue  # sanity; shouldn't happen since probe had ≥1

            # Update tallies & print compact line
            solved_count += 1
            total_solutions += len(combined_unique)
            family = f"{nH}H{nV}V"
            print(f"W{w}×H{h} • {family} → {len(combined_unique)} solution{'s' if len(combined_unique)!=1 else ''}")

            # Write only solved grids
            write_grid_block(f, layout, slots, intersections, combined_unique, phrase)
            f.write("-"*64 + "\n\n")

            # Check caps after writing
            rem_global = remaining_global_cap(total_solutions)
            if rem_global is not None and rem_global <= 0:
                if cli_cap is not None and total_solutions >= cli_cap:
                    print(f"Reached CLI global cap of {cli_cap} solutions. Stopping early.")
