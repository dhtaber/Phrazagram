...
function TopRibbonTile({
  ...
  isCorrect,    // boolean
  isPunctuation,
  isFirstInRow = false, // boolean (true for punctuation; spaces should be filtered upstream)
  tightLeft = false,         // <— add this
}) {
  // Colors
  ...
  // Balance punctuation so rows keep their full width (still tight to the left).
  const punctLeft = (isPunctuation && !isFirstInRow && ch) ? (ch === "." ? -8 : -8) : 0;
  const punctRight = (isPunctuation && !isFirstInRow && ch)
    ? ((ch === "'" || ch === "’" || ch === '"' || ch === "“" || ch === "”") ? 0 : (ch === "." ? 10 : 8))
    : 0;
  ...
  const wrapStyle = {
    ...
    padding: isPunctuation ? "0 1px" : (tightLeft ? "0 1px" : "0 3px"),
    marginLeft: punctLeft,
    marginRight: punctRight,
  ...
  // Nudge quotes left inside their tile (doesn't affect layout width)
  const isQuote =
    isPunctuation &&
    (ch === "'" || ch === "’" || ch === '"' || ch === "“" || ch === "”");
  const quoteShiftX = isQuote ? 0 : 0; // try -2px first

  const afterQuoteShiftX = (!isPunctuation && tightLeft) ? 1 : 0; // +1px for letters right after a quote
  const glyphShiftX = quoteShiftX + afterQuoteShiftX;
  ...
  const letterStyle = {
    ...
    paddingBottom: 0,
    pointerEvents: "none",
    transform: glyphShiftX ? `translateX(${glyphShiftX}px)` : undefined,
  };
...
>
          ...
          <div className="text-base font-semibold text-gray-900">Word Game Name Here</div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="relative z-10 w-[90%] max-w-sm rounded-2xl bg-white p-4 shadow-xl">
                ...
                <div className="text-sm text-gray-600">
                  Placeholder content. (stats/yesterday?/TBD)
                </div>
...
<div className="space-y-3 text-gray-800">
  <p>
    <span className="font-semibold">Objective:</span> Solve the phrase in the
    <span className="font-semibold"> fewest moves</span>. Each swap =
    <span className="font-semibold"> 1 move</span>.
    <span className="font-semibold"> </span> 
  </p>

  <ul className="list-disc pl-5 space-y-2">
    <li><span className="font-semibold">Swap letter tiles in the bottom grid.</span></li>
    <li>
      <span className="font-semibold">Use both grids together.</span> Build words on the
      bottom or read the phrase on top—whatever helps.
    </li>
    <li>
      <span className="font-semibold">Follow the little numbers under the top letters.</span>
      {" "}Each number is that letter’s home. Move tiles so the right letter sits over its number.
    </li>
    <li>
      <span className="font-semibold">When a letter reaches its home, it locks.</span>
      {" "}Keep swapping until everything locks.
    </li>
  </ul>
</div>
                <div className="mt-4 flex justify-end">
        ...
        {(() => {
          ...
          const row = { display: "flex", alignItems: "center", justifyContent: "center", gap: 2, width: "100%" };
          const deco = {
              ...
              {lines.map((items, li) => (
                <div key={`row-${li}`} style={row}>
                  {items.map((t, i) => {
                    ...
                    if (t.type === "space") {
                      if (i === 0) return null; // ← don't render a leading space
                    ...
                    if (t.type === "letter") {
                      ...
                      let ch = mask === mask.toLowerCase() ? base.toLowerCase() : base.toUpperCase();
                      if (isLocked) ch = displayCharByTopIndex[k] || ch;

                      // Copy the accent from the mask if it's the same letter (ignoring accents)



                      // is the previous item a quote punctuation?
                      const prev = items[i - 1];
                      const tightLeft = !!(prev && prev.type === "punct" && ["'", "’", '"', "“", "”"].includes(prev.ch));
      ...
      {/* Move counter (larger) */}
      <div className="text-xl text-gray-800 mt-2 leading-none">
        Moves: <span className="font-extrabold text-gray-900 text-2xl">{moveCount}</span>
      </div>

      {/* Par line */}
      <div className="text-sm text-gray-500 -mt-4">minimum moves needed to solve: <span className="font-semibold text-gray-700">{puzzle.min_swaps}</span></div>
