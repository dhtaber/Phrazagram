...
# - Includes Hungarian algorithm, scramble with K seeded greens, duplicate detection, atomic write & timestamped backups.
# - Default puzzles.json: C:\Alloquest_Photos\panama-v2\public\puzzles.json
#
# Usage:
#   Double-click or: python Generate_puzzles_Json_Solo.py
...
#   - start_state.scramble is a permutation of 1..N with exactly K seeded greens.
#   - start_state.correct_positions are locations (1..N) that begin correct.
...
# ---------------- Scramble with exactly K seeded greens ----------------

def generate_scramble_with_k_greens(target_for_tile: List[int], k_correct: int = 3, max_attempts: int = 6000) -> Tuple[List[int], List[int]]:
    N = len(target_for_tile)
    if k_correct < 0 or k_correct > N:
        raise ValueError("k_correct out of range")
    indices = list(range(1, N+1))

    def matches(scr: List[int]) -> int:
        return sum(1 for i in range(N) if scr[i] == target_for_tile[i])

    for _ in range(max_attempts):
        correct_tiles = set(random.sample(indices, k_correct)) if k_correct > 0 else set()
        correct_locations = { target_for_tile[t-1] for t in correct_tiles }
        remaining_tiles = [t for t in indices if t not in correct_tiles]
        pool_locations = [loc for loc in indices if loc not in correct_locations]

        ok = False
        for _retry in range(2000):
            random.shuffle(pool_locations)
            valid = True
            for t, loc in zip(remaining_tiles, pool_locations):
                if loc == target_for_tile[t-1]:
                    valid = False
                    break
            if valid:
                ok = True
        ...
        scramble = [0]*N
        for t in correct_tiles:
            scramble[t-1] = target_for_tile[t-1]
        for t, loc in zip(remaining_tiles, pool_locations):
            scramble[t-1] = loc

        if matches(scramble) == k_correct:
            correct_positions = sorted([target_for_tile[t-1] for t in correct_tiles])
            return scramble, correct_positions

    raise RuntimeError("Could not generate scramble with exact K seeded greens")
...
def calculate_min_moves(scramble: List[int], phrase_letters: str, target_for_tile: List[int]) -> int:
    N = len(phrase_letters)
    tile_at_location = [0]*(N+1)
    for t in range(1, N+1):
        tile_at_location[scramble[t-1]] = t

    needed_locations = []
    for loc in range(1, N+1):
        tgt_tile = next((t for t in range(1, N+1) if target_for_tile[t-1] == loc), None)
        if tgt_tile is None: raise RuntimeError("Internal mapping error")
        if tile_at_location[loc] != tgt_tile:
            needed_locations.append(loc)

    misplaced_tiles = [t for t in range(1, N+1) if scramble[t-1] != target_for_tile[t-1]]
    m = len(misplaced_tiles)
    if m == 0: return 0

    # Required letter at a location = letter of the tile whose target is that location
    def letter_of_tile(t: int) -> str: return phrase_letters[t-1]
    ...
    for loc in needed_locations:
        ...
        req_letter_at_loc[loc] = letter_of_tile(t_req)

    candidate_cols = sorted(needed_locations)
    BIG = 10**6
    c = len(candidate_cols)
    base = [[BIG]*c for _ in range(m)]
    for i, t in enumerate(misplaced_tiles):
        t_letter = letter_of_tile(t); cur_loc = scramble[t-1]
        for j, loc in enumerate(candidate_cols):
            base[i][j] = 0 if (t_letter == req_letter_at_loc[loc] and cur_loc == loc) else (1 if t_letter == req_letter_at_loc[loc] else BIG)
    ...
    for i, t in enumerate(misplaced_tiles):
        cur_loc = scramble[t-1]
        j = assignment[i]
        if j < c:
            loc_map[cur_loc] = candidate_cols[j]

    visited = set(); cycles = 0; elements = 0
    for start in list(loc_map.keys()):
        if start in visited: continue
        cur = start; clen = 0
        while cur not in visited and cur in loc_map:
            visited.add(cur); clen += 1; cur = loc_map[cur]
        if clen > 0: cycles += 1; elements += clen
    return max(0, elements - cycles)
...
def build_puzzle_object(phrase: str, theme: str, grid: GridSpec, words: List[WordSpec], seeded_greens: Optional[int]) -> Dict[str, Any]:
    ...
    scramble, correct_positions = generate_scramble_with_k_greens(target, k_correct=k)
    min_moves = calculate_min_moves(scramble, phrase_letters, target)
    return {
        "phrase": phrase,
        ...
        "start_state": {"scramble": scramble, "correct_positions": sorted(correct_positions), "min_moves": int(min_moves)}
    }

def append_or_overwrite(puzzles_path: str, puzzle_obj: Dict[str, Any]) -> str:
    data = load_or_init_puzzles(puzzles_path)
    # Determine default seeded from config for info only (already applied)
...
class App(tk.Tk):
    ...
    def on_append(self):
        ...
        # Build puzzle object (validates & computes)
        try:
        ...
        # Append/overwrite with backup + atomic write
